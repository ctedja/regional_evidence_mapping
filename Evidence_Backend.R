...
evidence %>% pivot_longer(cols = paste0("c", seq_len(nmax)),
                          names_to = "Coverage_N",
                          values_to = "Coverage",
                          values_drop_na = TRUE) %>%
  select(-c(Coverage_N, Index, Theme, Source)) %>%
  mutate(Date = as.Date(as.numeric(Date), origin = "1899-12-30")) %>%
  mutate(Source = "Manual Entry", 
         Origin_Link = NA)
...
reliefweb_df <- bind_rows(lapply(reliefweb_list, as.data.frame)) %>%
  ...
  rename(Country = fields.country.name,
...
geography <- read_sf("C:\\Users\\clinton.tedja\\OneDrive - World Food Programme\\Documents (OneDrive)\\Data\\RBB_administrative_boundary_polygon_shapefile\\RBB_administrative_boundary_polygon\\RBB_bnd_adm2_WFP.shp")
